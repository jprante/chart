...
import org.xbib.graphics.chart.io.BitmapEncoder;
...
public class SwingWrapper {
    ...
    public JFrame displayChart() {
        ...
        SwingUtilities.invokeLater(() -> {
            ...
            JPanel chartPanel = new ChartPanel<Chart>(charts.get(0));
    ...
    private JFrame displayChartMatrix() {
        ...
        SwingUtilities.invokeLater(() -> {
            ...
            for (Chart chart : charts) {
                if (chart != null) {
                    JPanel chartPanel = new ChartPanel<Chart>(chart);
                    frame.add(chartPanel);
                } else {
    ...
    class ChartPanel<T extends Chart> extends JPanel {
                ...
                if (fileChooser.getSelectedFile() != null) {
                    ...
                    try {
                        ...
                        if (fileChooser.getFileFilter() == null) {
                            ...
                            BitmapEncoder.saveBitmap(chart, outputStream, BitmapEncoder.BitmapFormat.GIF);
                        } else if (fileChooser.getFileFilter().getDescription().equals("*.svg,*.SVG")) {
                            VectorGraphicsEncoder.write(chart,outputStream, VectorGraphicsEncoder.VectorGraphicsFormat.SVG);
                        } else if (fileChooser.getFileFilter().getDescription().equals("*.eps,*.EPS")) {
                            VectorGraphicsEncoder.write(chart, outputStream, VectorGraphicsEncoder.VectorGraphicsFormat.EPS);
                        } else if (fileChooser.getFileFilter().getDescription().equals("*.pdf,*.PDF")) {
                            VectorGraphicsEncoder.write(chart, outputStream, VectorGraphicsEncoder.VectorGraphicsFormat.PDF);
                        }
                    } catch (IOException e) {
                        e.printStackTrace();
                    }
